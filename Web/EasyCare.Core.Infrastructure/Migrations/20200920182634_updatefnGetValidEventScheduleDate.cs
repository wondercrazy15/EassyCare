...
{
    public partial class updatefnGetValidEventScheduleDate : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
			string procedure = @"ALTER FUNCTION [dbo].[fnGetValidEventScheduleDate]
(
...
BEGIN
	...
	BEGIN
		SET @Date = DATEADD(minute, -10,@Date)
	END

	ELSE IF dbo.fnGetPeriodOfTime(@PeriodOfTime) = 4
	BEGIN
		SET @Date = DATEADD(minute, -15,@Date) 
	END
